...
class SiteStatisticFix(_PluginBase):
    # 插件名称
    plugin_name = "站点数据统计修改版"
    # 插件描述
    plugin_desc = "临时修复原版bug, 不长期更新(需要先安装原版,可以不运行)"
    # 插件图标
    plugin_icon = "statistic.png"
    # 插件版本
    plugin_version = "3.1"
    # 插件作者
    plugin_author = "lightolly,wlj"
    # 作者主页
    author_url = "https://github.com/developer-wlj"
    # 插件配置项ID前缀
    plugin_config_prefix = "sitestatisticfix_"
    # 加载顺序
    plugin_order = 1
    # 可使用的用户级别
    auth_level = 2
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._enabled and self._cron:
            ...
            for trigger in triggers:
                ret_jobs.append({
                    "id": f"SiteStatisticfix|{trigger.hour}:{trigger.minute}",
                    "name": "站点数据统计服务修改版",
                    "trigger": "cron",
                    "func": self.refresh_all_site_data,
                    "kwargs": {
